...
def manager_dashboard(request):
    if request.user.is_admin:
        ...
        order_new = Order.objects.filter(status='New').count()
        order_accepted = Order.objects.filter(status='Order Accepted').count()
        order_delivered = Order.objects.filter(status='Delivered Successfully').count()
        order_cancelled = Order.objects.filter(status='Order Cancelled').count()
...
def manage_category(request):
    if request.user.is_admin:
        if request.method == 'POST':
            keyword = request.POST['keyword']
            categories = Category.objects.filter(Q(categoryName__icontains=keyword) | Q(slug__icontains=keyword)).order_by('id') 
